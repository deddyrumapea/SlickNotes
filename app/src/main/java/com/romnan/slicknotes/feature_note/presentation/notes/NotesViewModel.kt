...
class NotesViewModel @Inject constructor(private val noteUseCases: NoteUseCases) : ViewModel() {
    ...
    private var findAllJobs: Job? = null

    init {
        findAll(NoteOrder.Date(OrderType.Descending))
    }
    ...
    fun onEvent(event: NotesEvent) {
        when (event) {
            ...
            is NotesEvent.RestoreNote -> {
                viewModelScope.launch {
                    noteUseCases.save(recentlyDeletedNote ?: return@launch)
    ...
    private fun findAll(noteOrder: NoteOrder) {
        findAllJobs?.cancel()
        findAllJobs = noteUseCases.findAll(noteOrder)
            .onEach { notes ->
                _state.value = state.value.copy(
                    notes = notes,
                    noteOrder = noteOrder
